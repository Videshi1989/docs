...
const AdminContactUs = () => {
	...
	const Pagesubheading = 'Home / Contact-Us';
	...
	const getAllRecords = async ()=>
	{
		...
		try 
		{
			const response = await fetch(`${ADMINAPI}/contectedusers`,{
				method:"GET",
				headers:{
	...
	async function ShowpopupView(getid)
	{
	  ...
	  try{
		const response = await fetch(`${ADMINAPI}/getsinglecontacteduser/${getid}`,{
			method : 'GET',
			headers:{
						'Content-Type': 'application/json',
						Authorization : authorizationToken 
...
function downloadCSV(args) {  
    ...
    const columns=[
        {
            name:"NAME",
            selector:row=>row.contactname,   //fullname is colum name of table
            sortable:true,
        ...
        {
            name:"MOBILE NO.",
            selector:row=>row.contactmobileno,
            sortable:true
        }
        ,
        {
			...
			cell: cellinfo =>([<HiEye title='View' className='actioniconview' onClick={()=> ShowpopupView(cellinfo._id)} />,<MdEdit title='Edit' style={{opacity:'0.5',pointerEvents:'none',display:'none'}} className='actionicon' onClick={()=> ShowpopupEdit(cellinfo._id)}  />, " ", <RiDeleteBin6Line title='Delete' style={{pointerEvents:'none',opacity:'0.5'}} className='actionicon' onClick={()=> ShowpopupDelete(cellinfo._id)} />]),
		
        }
       ];
	   const columnspdf = [
		{ header: "NAME", dataKey: "contactname" },
		{ header: "EMAIL", dataKey: "contactemail" },
		{ header: "MOBILE", dataKey: "contactmobileno" },
		{ header: "MESSAGE", dataKey: "contactmsg" },
       ...
       function handleFilter(event)
       {
			...
			else
			{
				if(event.target.value !== '')
            	{
                    ...
                    return (row.contactname && row.contactemail).toLowerCase().includes(event.target.value.toLowerCase());
					});*/
					const newData = allrecords.filter(row=>{
						return(
							   row.contactname.toLowerCase().includes(event.target.value.toLowerCase()) ||
							   row.contactemail.toLowerCase().includes(event.target.value.toLowerCase()) ||
							   row.contactmobileno.toString().toLowerCase().includes(event.target.value.toLowerCase())
							  );
					   });
                	setRecords(newData);
					setdatatblloader(false);
	...
	//////for Edit Record
   	const[edituserdata,Setedituserdata] = useState({fullname:"", email:"", mobileno:""});
	...
	const DeleteRecord = async(e)=>{
		...
		try
		{
			const response = await fetch(`${ADMINAPI}/deletecontactrecord/${deleteid}`,{
				method : 'DELETE',
				headers:{
							'Content-Type': 'application/json',
							Authorization : authorizationToken 
	...
	const FilterByDate = async ()=>{
		...
		try
		{
			const response = await fetch(`${ADMINAPI}/filtercontactedusersbydate`,{
				method:"POST",
				headers:{
					'Content-Type':'application/json',
					Authorization : authorizationToken
	 ...
	 <div className="viewheight viewheightmobile" id="style-scroll2">
		<div className="row mt-3">
			...
			<div className="col-10 mobwidth100admin">
				<span className="cont-cont">{viewdata.contactname}</span>
			</div>
		...
		<div className="row mt-3">
			...
			<div className="col-10 mobwidth100admin">
				<span className="cont-cont">{viewdata.contactemail}</span>
			</div>
		</div>
		<div className="row mt-3">
			<div className="col-2 mobwidth100admin">
			...
			<div className="col-10 mobwidth100admin">
				<span className="cont-cont">{viewdata.contactmobileno}</span>
			</div>
		</div>
		<div className="row mt-3">
			<div className="col-2 mobwidth100admin">
				<span className="cont-heading">Message:</span>
			</div>
			<div className="col-10 mobwidth100admin">
				<span className="cont-cont">{viewdata.contactmsg}</span>
			</div>
		</div>
